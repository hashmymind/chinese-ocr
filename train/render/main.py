...
from render.libs.config import load_config
from render.libs.timer import Timer
from render.parse_args import parse_args
import render.libs.utils as utils
import render.libs.font_utils as font_utils
from render.textrenderer.corpus.corpus_utils import corpus_factory
from render.textrenderer.renderer import Renderer
...
flags = parse_args()
cfg = load_config('render/'+flags.config_file)

fonts = font_utils.get_font_paths_from_list('render/'+flags.fonts_list)
bgs = utils.load_bgs('render/'+flags.bg_dir[2:])

corpus = corpus_factory(flags.corpus_mode, flags.chars_file, flags.corpus_dir, flags.length)

renderer = Renderer(corpus=corpus, fonts=fonts, bgs=bgs,cfg= cfg,
